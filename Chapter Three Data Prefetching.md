...
通过改变索引表中存储的键和历史缓冲区条目之间的链接指针，GHB设计可以利用与触发事件相关的各种预测前缀流属性。Nesbit 和 Smith 引入了 GHB 变体的分类，形式为 GHB X/Y，其中X表示流如何局部化（即，链接指针如何连接应连续预读的历史缓冲区条目），Y 表示相关方法（即查找过程如何定位候选流）[68,69]。局部化可以是全局（G）或每台计算机（PC）。在全局局部化下，按顺序记录的历史缓冲器条目形成一个流。每个历史表条目的指针要么指向同一缺失地址的较早出现（如上所述，有助于更高的预取宽度），要么未使用。在按 PC 局部化的情况下，索引表和链接指针都根据触发访问的 PC 将历史缓冲器条目连接起来；一条流是由链接指针连接的同一个触发 PC 发出的连续错误所形成的。相关方法可能是地址相关（AC）或微分相关（DC）。在本节中，我们讨论全局地址相关变体 (GHB G/AC)，其中索引表将缺失地址映射到历史缓冲器位置。在第 3.3.2 节中，我们讨论了 GHB PC/DC（“程序计数器局部化微分相关”），它找到并记录他的- 17 -
